...
- Jest test implementation uses a three-stage pipeline: `TestScaffold` (writes tests) → `TestReviewer` (static gate: checklist, tsc, eslint) → `TestRunner` (execution with hang detection). Always provide a behavior matrix from the actual implementation, including unsupported scenarios to avoid. Consult `docs/testing/projects/<project>.md` for that project's tooling and mock patterns (`docs/testing/README.md` is the index plus cross-project rules). Max 2 reject-cycles before escalating to the main agent (ADR 0017). Hitting the cap, a repeated FAIL, or a static PASS then Runner/`tsc`/`eslint` FAIL is a Pipeline Incident — comment `## Pipeline Incident` on the Slice Issue. `/code-review` runs once per Slice after deterministic checks are green, not after every specialist hop.
...
| Components in `libs/web-ui/` / `libs/web/pages/` | Component workflow (below)                              | ComponentBuilder → ComponentReviewer (max 2 FAIL cycles; ADR 0017)                                                                          |
...
- **`FAIL`** — relay `Required Revisions` to ComponentBuilder and repeat from Step 2. **Max 2 FAIL cycles**, then escalate to the main agent/human with a diagnosis (ADR 0017). File a Pipeline Incident when the cap is hit, the FAIL repeats, or a sibling already solved it. Prefer short reviewer reports (`PASS|FAIL` + ≤5 bullets) unless detail is needed after a rejection.
