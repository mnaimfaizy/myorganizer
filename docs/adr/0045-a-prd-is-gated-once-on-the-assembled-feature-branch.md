...
**Keep per-slice gates but make them non-blocking** was rejected as the primary design, though it is close. It preserves per-slice attribution, which the chosen design gives up. But it keeps all N installs, and — decisively — it still cannot see cross-slice breakage, so it would leave the PRD ungated at the scope that actually ships. Retained in spirit: the per-slice verdict still reaches the log as a `HANDOFF:` marker ([ADR 0044](0044-a-resumed-slice-is-told-what-the-previous-run-did.md)) when a gate does run.
