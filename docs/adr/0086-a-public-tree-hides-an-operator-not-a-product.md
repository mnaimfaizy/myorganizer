...
One genuine fingerprint was found in source: `myorganizerapi.mnfprofile.com`, in `apps/backend/src/config/http.ts` and three times in `apps/backend/src/main.test.ts`. It is not a product URL. It ties MyOrganizer's API host to the operator's personal web presence, which is the category the issue was about.
...
- **`myorganizerapi.mnfprofile.com` leaves the source**, replaced by the per-environment `CORS_ORIGINS` that `.env.example` and `docs/deployment/CPANEL_BACKEND_HOSTING.md` already declare. The value of that edit is stopping accretion, not erasing the past.
...
- **A denylist checker naming forbidden fingerprints** — rejected on its face. A file naming `mnfprofile.com` as forbidden publishes, in a public repository, the exact value it protects.
