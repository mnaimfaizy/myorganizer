...
- **2026-09-12 addendum**: Issue [#290](https://github.com/mnaimfaizy/myorganizer/issues/290) closed the other half of the same failure. Once the Vault UI Component scope (ADR 0026) was wired into `scopeOf()`, the scan roots grew. The checker scope is deliberately **wider than the category GUIDELINES §1 names**: it is every `.tsx` under `libs/web-vault-ui/src/lib/`, so `session`, `vaultGate` and `reconcileRunner` are checked even though §1 lists them as not Vault UI Components. That section excludes them from _needing stories_, which is a question about presentational purity; whether an effect cleans up after itself and whether an import goes through the barrel are not questions about purity, and they are the last questions to leave unasked in the files that gate the vault. The narrowing that does apply is the rule list: the `vault-ui` scope omits the primitive-only rules. `.ts` modules in the same flat directory stay out, because a hook reporting PASS as a component is the same false clean this scope was added to remove. Re-baselined, the full scan is **207 files inspected, 0 errors, 0 warnings** (120 stories and test files skipped) — the zero-warning baseline above still holds, over a wider corpus. The 140-component figure under Calibration is the original pre-#288 measurement and is left as recorded history. The exit-code contract gained a case: **a run in which every input fell out of scope now exits 1**. Previously such a run printed `0 error(s), 0 warning(s)` and exited 0, indistinguishable from a clean pass, which is precisely how `libs/web-vault-ui` sat unchecked behind a checker that looked green — a skip that reads as a pass is worse than no check. `0` now means checks ran and passed, never that there were no checks to run. A mixed run is unaffected and still exits on the merits of the files it did check, because there a skip is the scope filter working rather than an invocation mistake; `--staged` with nothing staged still exits 0, because that is a legitimate state. Contract tests cover the new scope, its flat-barrel path resolution, and the all-skipped exit code in both text and JSON modes.
