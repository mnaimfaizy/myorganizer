...
> Last synced from `package.json` on 2026-08-28.
...
| ---------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| 1138808, 1138809 | `image-size`      | Revalidated 2026-08-15. Patched in 2.0.3, but `metro@0.82.5` needs `^1.0.2` and calls it as a default export, which v2 removed; forcing 2.x breaks the React Native asset pipeline. `less@4.5.1` needs `~0.5.0` and marks it optional. Both are build-time tools over first-party assets, so the parser DoS is not reachable from untrusted runtime input. | metro/React Native upgrade, a new advisory that is not build-time-only, or 2027-02-15 | [#285](https://github.com/mnaimfaizy/myorganizer/issues/285) |
