...
- For instruction-truth and breaking-change discovery against official docs, use `.github/skills/upstream-brief/SKILL.md`. The human names each `subject@version`; the run writes an Upstream Brief and may propose a HITL issue. It does not bump packages or apply instruction edits. Host adapter: `upstream-brief.config.yml`. Contract: `docs/adr/0018-upstream-brief-portable-instruction-audit.md`.
