...
- Design token drift check: `yarn design-tokens:check` (rebuilds `libs/design-tokens/src/generated` from `tokens.json` and asserts the tree did not move — `openapi:check`'s shape, for the same reason. Until [ADR 0098](docs/adr/0098-a-covering-gate-is-part-of-the-site-not-the-answer.md) needed a Covering Gate for the tokens trigger, nothing asserted the two agreed: `tailwind:classes:check` covers a utility naming a theme value that compiles to no CSS, which is consumption and not regeneration, so a token whose _value_ changed without a rebuild was caught by nothing. CI-only rather than in the pre-commit aggregate, because it runs a build).
...
- Nx declared-target ratchet check: `yarn nx:targets:check` (asserts every `project.json` target on an `@nx/*` executor — a Declared Target Nx v24 removes in favour of plugin-Inferred Targets — matches `tools/config/nx-declared-targets-baseline.json` exactly, keyed by project, target, and executor; a target not already in the baseline fails, and a baseline entry with no matching target fails as stale, so the baseline can only shrink. `nx:run-commands` targets, executors outside the `@nx/` namespace, and executor-less overrides are out of scope. An `@nx/*` executor Nx has not deprecated is not debt: the file's `notDebt` list names it with a reason and a source, and an entry covering no target fails as stale (`@nx/js:node` is the only one). See [ADR 0082](docs/adr/0082-nx-targets-are-inferred-and-a-declared-target-is-migration-debt.md) and [ADR 0083](docs/adr/0083-a-declared-target-is-debt-only-on-an-executor-nx-deprecated.md). `yarn nx:targets:test` covers the checker.
...
- Fix attribution check: `yarn fix:attribution:check --base <sha> --head <sha> --branch <ref> [--title <text>]` (a Wired Gate run by CI on every Pull Request — a fix's commits must carry `Introduced in #<pull request>`, or `Introduced in unknown: <why>` when nothing can be named, the line the escaped-defect rate is built from; it asks the measurement's own parser rather than restating it, and does not check that the reference resolves — see [ADR 0100](docs/adr/0100-a-fix-names-what-introduced-it-in-its-commits.md)). `yarn fix:attribution:test` covers the checker.
...
- Documented file-ref check: `yarn docs:file-refs:check` (asserts that a backticked repo-relative path, or a camelCase/PascalCase module name in the same sentence as a trailing-slash `libs/` or `apps/` directory, resolves on disk; `docs/research/` is skipped as frozen history; remaining recorded-history claims carry a written reason in `tools/config/doc-file-refs-exemptions.json`, and a stale entry fails — see [ADR 0093](docs/adr/0093-a-markdown-file-ref-is-asserted-against-the-tree.md)).
- Escape Copy reader build: `yarn escape-copy-reader:build` (bundles `apps/escape-copy-reader` into one self-contained HTML file plus `SHA256SUMS.txt` under `dist/escape-copy-reader/`; `.github/workflows/publish-github-release.yml` runs it and attaches both to every GitHub Release).
- Escape Copy reader gate: `yarn escape-copy-reader:check` (a Wired Gate run by CI — builds the reader, produces an envelope **here and now** with the real `exportVault`, and opens it with the **built artifact** rather than the sources it came from, under both the passphrase and the Recovery Key, comparing every Vault Blob Type's plaintext. It also fails a reader built for a schema version the exporter has moved past ([ADR 0064](docs/adr/0064-an-escape-copy-is-opened-by-a-tool-that-needs-nothing-of-ours.md) decision 2 — "a reader pinned to a schema version the exporter has moved past is worse than no reader: it fails at the only moment anyone runs it"), a built page carrying any way to reach the network or browser storage, a built page that spaces itself off the design tokens in name only — a `var(--name)` naming a token the pipeline does not emit, which silently renders as its fallback, or a spacing literal equal to a value a token already carries (the reader shipped with both halves wrong) — and a published checksum that is not the checksum of the published file. CI-only rather than in the pre-commit aggregate: it builds a 500 KB page and runs PBKDF2 four times. The judgments live in `tools/scripts/lib/escape-copy-reader-gate.mjs`; `yarn escape-copy-reader:test` covers them.
- Mobile platform check: `yarn mobile-platform:check` (parses `apps/mobile` and `libs/mobile` source and fails a bare `react-native/…` subpath import — deprecated at 0.80 with removal planned, and never caught here because `@react-native/eslint-config` is not installed — or a browser global (`localStorage`, `sessionStorage`, `window`, `document`, `crypto.subtle`), which the compiler now also rejects in every native mobile program (no `dom` in `lib`; the app's web program keeps it for `react-native-web`), but only for files a program includes, so the gate still scans every mobile source file — or an import of a shared library's main entry point when `tsconfig.base.json` declares a `@myorganizer/<lib>/portable` alias for it, the Portable Entry Point rule of [ADR 0103](docs/adr/0103-mobile-native-code-is-typechecked-without-dom-and-reaches-shared-libraries-through-a-portable-entry-point.md), whose library list is derived from those aliases and never written down; `--print` lists what was scanned and the Portable Entry Points it derived. The subpath match excludes the separate `@react-native/*` scope and covers all four call forms — `from '...'`, `require('...')`, `require.resolve('...')`, and dynamic `import('...')`. Exemptions carry a written reason in `tools/config/mobile-platform-exemptions.json`; a stale entry fails the check.
- Committed Upstream Brief reports: `yarn upstream:briefs:check` (re-validates every structured report committed in the brief directory — every `*.json` at its top level, because a report that escaped the gate through a typo'd name is the failure the Meta-Gate exists to prevent one level down). A run that writes a structured report renders its brief from that report and commits both, which is what makes a frozen brief checkable: every local citation is a `file`, a `line`, and the literal text at that line, read back with `git show <commit>:<file>` at **the commit the report itself records** rather than the tree in front of it ([ADR 0084](docs/adr/0084-an-upstream-brief-is-anchored-to-what-is-installed-and-accepted-on-checked-evidence.md) items 3 and 10). Where a code review rejects a report whole, this one moves the refused entry to an **Unverified** list with a named reason and the rest of the report stands — a review computes a verdict one bad finding corrupts, a brief has no verdict, and losing an Ecosystem's good findings to one bad citation costs more. The validator itself ships inside the skill (`.agents/skills/upstream-brief/report.mjs`, with `render.mjs` and the `validate-report.mjs` CLI beside it) and imports nothing outside Node built-ins, because ADR 0018 keeps the skill portable; `tools/scripts/check-upstream-briefs.mjs` is a thin adapter that exists so the Meta-Gate — which scans `tools/scripts/` non-recursively — can see the gate at all. The same gate also holds the adapter's **declined Upstream Opportunities** (ADR 0084 item 12) to the other tree — the current one: an entry naming an Ecosystem the adapter no longer declares, or a local site path no longer present, fails, because it then silences nothing and records a decision about something that is not there. A decline is live configuration, not a frozen document. It is deliberately not required to match an Opportunity any run proposed: suppression is decided at run time against the Baseline and the upstream quote, so a gate demanding a live match would fail on exactly the entry doing its job. Exit 1 on an invalid report, local evidence that no longer matches, or a declined entry pointing at nothing, exit 2 on a recorded commit this clone does not have (told apart deliberately: `git show` fails identically for a missing path and an unresolvable ref, so a shallow clone would otherwise report every citation as a fabrication). A brief directory holding no structured report passes and says so — the three briefs written before ADR 0084 are not migrated. `yarn upstream:briefs:test` covers the contract, the renderer, the CLIs, the ledger, and the checker; [REPORT.md](.agents/skills/upstream-brief/REPORT.md) is the shape and [LEDGER.md](.agents/skills/upstream-brief/LEDGER.md) is what a run remembers from the last one — which checked-and-clear claims carry forward to a new Baseline (`yarn upstream:ledger` prints it), and which Opportunities stay declined. The skill's own workflow now writes the report, validates it, renders the brief from it, and commits both ([SKILL.md](.agents/skills/upstream-brief/SKILL.md) step 8), so every run from here on leaves this gate something to re-validate. The three briefs written before ADR 0084 are not migrated, so until a run happens the gate finds no report and says so.
- Prisma migration history check: `yarn prisma:migrations:check` (the file-reading half of the Prisma migration gate — asserts every entry beside `migration_lock.toml` is a directory named `<14-digit timestamp>_<lowercase_snake_slug>`, that no two migrations share a timestamp, that every migration directory carries a `migration.sql` holding a statement once SQL comments are stripped, that no other `.sql` file sits beside it where the engine will never read it, and that the lock file's provider agrees with the schema's datasource; `--print` lists what it read. It deliberately asserts **nothing** about whether the SQL parses, applies, or reproduces the schema — that needs a database and is the `Test` job's `prisma migrate deploy` plus `prisma migrate diff --exit-code` against its ephemeral Postgres service. Passing this one is not evidence a migration works; it is evidence the history is well-formed enough for the job that decides to be able to run it. Two branches that each write a migration never conflict in git, which is how two can land on the same timestamp and the engine's order become whichever name sorts first — the hazard ADR 0042 records for ADR numbers. See [ADR 0094](docs/adr/0094-a-prisma-migration-is-gated-on-the-tree-and-against-a-database.md); `yarn prisma:migrations:test` covers the checker.
...
- Checker contract-coverage check: `yarn checker-contracts:check` (asserts every `tools/scripts/check-*.mjs` either carries a contract suite — a sibling `check-*.test.mjs` proving it fails on the drift its header claims to catch — or is named in the shrink-only baseline `tools/config/checker-contract-baseline.json`; ADR 0043's "state your direction in the header" clause is not enough on its own, because `check-readme.mjs` followed it in exactly the fixed form asked for and still implemented one direction of the two it claimed — the header is the specification, the contract suite is what makes it true (ADR 0085)). A checker with neither a suite nor a baseline entry fails as new, untracked debt; a baseline entry naming a checker that is no longer debt — it gained a suite, or the checker itself is gone — fails as stale, so the baseline can only shrink. Deliberately no written-reason opt-out, unlike the Meta-Gate above: a per-checker "not worth testing" judgment call is exactly the discretion this baseline exists to remove. Same ratchet shape as the Declared Target check (`yarn nx:targets:check`). `yarn checker-contracts:test` covers the checker.
...
- House Explainer Page citations: a `file:line` citation on a page carries an expected-content anchor in the page's `citation-anchors` block, and `yarn design:hygiene` resolves the citation **and** compares the line against the anchor (ADR 0085). An anchor whose own `file` cannot be read, or whose cited line is past that file's end, fails as `citation-anchor-unreadable` rather than passing unverified. The requirement is per citation, not per page: a page carrying no anchor block at all is held to it exactly like one that opted in, because a rule keyed off the block would let a page pass by carrying nothing. `LEGACY` does not exempt a page from it — that exemption covers mechanical hygiene only. Pages whose citations are not yet anchored are named in the shrink-only baseline `tools/config/citation-anchor-baseline.json`; a page whose citations are all anchored fails as a stale entry, and there is no written-reason opt-out. `yarn design:hygiene:test` covers the rules and replays the drift #771 corrected against the real page and the real history.
