...
- Mobile platform check: `yarn mobile-platform:check` (parses `apps/mobile` and `libs/mobile` source and fails a bare `react-native/…` subpath import — deprecated at 0.80 with removal planned, and never caught here because `@react-native/eslint-config` is not installed — or a browser global (`localStorage`, `sessionStorage`, `window`, `document`, `crypto.subtle`), which typechecks today because the base TypeScript config puts `dom` in `lib`; `--print` lists what was scanned. The subpath match excludes the separate `@react-native/*` scope and covers only `from '...'` and literal `require('...')`, not `require.resolve(...)`. Exemptions carry a written reason in `tools/config/mobile-platform-exemptions.json`; a stale entry fails the check.
