...
- House Explainer Page citations: a `file:line` citation on a page carries an expected-content anchor in the page's `citation-anchors` block, and `yarn design:hygiene` resolves the citation **and** compares the line against the anchor (ADR 0085). The requirement is per citation, not per page: a page carrying no anchor block at all is held to it exactly like one that opted in, because a rule keyed off the block would let a page pass by carrying nothing. `LEGACY` does not exempt a page from it — that exemption covers mechanical hygiene only. Pages whose citations are not yet anchored are named in the shrink-only baseline `tools/config/citation-anchor-baseline.json`; a page whose citations are all anchored fails as a stale entry, and there is no written-reason opt-out. `yarn design:hygiene:test` covers the rules and replays the drift #771 corrected against the real page and the real history.
