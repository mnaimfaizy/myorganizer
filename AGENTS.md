...
- CI code review: `.github/workflows/code-review.yml` runs the `/code-review` skill on every non-draft Pull Request as the `Agent Verdict` check (ADR 0069 item 6; ADR 0070). Plumbing scripts: `yarn review:spec` resolves the spec source with the job token, `yarn review:validate --tier <label>` pins the classifier's tier, `yarn review:publish` edits the sticky summary comment, posts inline comments for blocking findings, and relabels to `review:human` on a blocking verdict. The reviewer writes JSON only and never holds a token. Requires the `ANTHROPIC_API_KEY` secret; without it the job explains and skips.
