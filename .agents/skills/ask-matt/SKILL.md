...
- **Something's broken** → **`/diagnosing-bugs`** (hard bugs, flakes, regressions). Refuse theories until there is a tight red-capable feedback loop; hand off to **`/improve-codebase-architecture`** when the finding is a missing seam. Prefer the repo skill at `.agents/skills/diagnosing-bugs/` when present.
