...
- **Before any focused-watching UI work, read `docs/features/youtube-prototype-references.md`** and the locked prototype variant it names for the surface you are touching. The Wayfinder map (#242) locked one variant per direction; a PRD or slice issue only summarises that lock, while the prototype defines the actual interaction model. Promote its decision-rich shapes; never copy its markup wholesale or ship a prototype route.
