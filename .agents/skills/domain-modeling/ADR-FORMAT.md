...
- **Status** frontmatter (`accepted | deprecated | superseded by ADR-NNNN`): useful when decisions are revisited. There is deliberately no `proposed`: an ADR is authored `accepted`, because a status that needs flipping in a later step does not get flipped. In this repo `yarn adr:status:check` asserts it (ADR 0097).
