...
- Do not invent labels. The runner rejects names that are not Surface Labels in `tools/config/github-labels.json` (ADR 0025); the one other name it accepts is the `agent-review` Request Label, which asks the CI code review to run on the new Pull Request (ADR 0069 item 7).
