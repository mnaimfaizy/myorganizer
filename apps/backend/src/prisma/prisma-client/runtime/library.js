...
function ui(e) {
  let t = e.e,
    ...
    i = `Please refer to the documentation about Prisma's system requirements: ${ii(
...
function ns(e) {
  if (ac(e)) {
    ...
    return {
      dotenvResult: rs(t),
      message: Oe(
        `Environment variables loaded from ${ht.default.relative(
          process.cwd(),
          e
        )}`
      ),
...
m.toFraction = function (e) {
  ...
  return (
    ...
    (l.s = u.s = f.s),
    (d =
      N(u, n, o, 1).minus(f).abs().cmp(N(l, r, o, 1).minus(f).abs()) < 1
        ? [u, n]
        : [l, r]),
    (h.precision = c),
    (x = !0),
    ...
    async withRetry(t) {
      for (let r = 0; ; r++) {
        ...
        try {
          ...
          this.logEmitter.emit('warn', {
            message: `Attempt ${r + 1}/${cl} failed for ${t.actionGerund}: ${
              i.message ?? '(unknown)'
            }`,
            timestamp: new Date(),
            target: '',
