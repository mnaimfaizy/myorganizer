...
- Treat a finding as valid only when it cites a **primary** upstream page for the *named target version* (official docs, vendor release notes, or a spec). The training corpus, blogs, and installed third-party skills are not sources.
...
If `source_globs` or `script_globs` are set, *sample* them for names the research hops return. Do not inventory the tree.
