...
description: >
  ...
  security-sensitive behavior. Invoke with a full delegation brief that includes
  source paths, target test paths, project name, and a behavior matrix.
...
1. Write the **first 2–3 test cases** (happy path and one error path).
2. Run them: `yarn nx test <project> --testNamePattern="<describe block>" --passWithNoTests`.
3. Fix any failures before proceeding.
4. Write the **next batch** (side effects, boundary, security paths).
...
- Any implementation dead code surfaced during testing (state declared but never exported or used — remove it from the source file only if permitted by the delegation brief).
